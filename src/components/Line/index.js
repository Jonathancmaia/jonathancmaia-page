...
const Line = () => {
  ...
  window.onscroll = () => {
    document.getElementById("line").style.height =
      30 + Math.ceil(scrollYProgress.current * 100) / 1.4 + "%";
    document.getElementById("lineBright").style.height =
      30 + Math.ceil(scrollYProgress.current * 100) / 1.4 + "%";
    ...
    for (let i = 0; i < titles.length; i++) {
      if (
        ...
        Math.ceil(
          document.getElementById("line").getBoundingClientRect().bottom
        )
      ) {
        if (
          !titles[i].classList.contains("title-active") &&
          window.innerWidth > 991
        ) {
          //Remove items if exists (prevents render bug on up and down fast scolling)
          if (document.getElementById("title" + i)) {
            document.getElementById("title" + i).remove();
          }

          if (document.getElementById("preTitle" + i)) {
            document.getElementById("preTitle" + i).remove();
          }
